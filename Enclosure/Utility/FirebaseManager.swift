...
class FirebaseManager: NSObject, ObservableObject {
    ...
    func requestNotificationPermissions() {
        ...
        UNUserNotificationCenter.current().getNotificationSettings { settings in
            ...
            if settings.authorizationStatus == .authorized {
                ...
                print("📱 [NOTIFICATION_PERMISSION] Requesting user consent for notifications...")
                self.showNotificationConsentDialog { granted in
                    if granted {
                        print("✅ [NOTIFICATION_PERMISSION] User consented - requesting authorization...")
                        ...
                        print("🚫 [NOTIFICATION_PERMISSION] User declined consent - notifications will be optional")
                    }
                }
            }
        }
    ...
    /// Show notification consent dialog to make notifications optional
    private func showNotificationConsentDialog(completion: @escaping (Bool) -> Void) {
        guard let topViewController = UIApplication.shared.topViewController() else {
            completion(true) // Fallback: proceed if can't show dialog
            return
        }
        
        let alert = UIAlertController(
            title: "Enable Notifications?",
            message: "Stay connected with friends! Enable notifications to receive messages and calls instantly. You can always change this later in Settings.",
