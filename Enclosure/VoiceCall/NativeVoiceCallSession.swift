...
final class NativeVoiceCallSession: ObservableObject {
    ...
    private func sendRemoveCallNotificationIfNeeded() {
        NSLog("📞 [NativeSession] removeCallNotification check: isCallConnected=\(isCallConnected), sent=\(removeCallNotificationSent), isSender=\(payload.isSender)")
        guard payload.isSender, !removeCallNotificationSent, !isCallConnected else { return }
        removeCallNotificationSent = true
        NSLog("📞 [NativeSession] removeCallNotification — sending (call was NOT connected)")
