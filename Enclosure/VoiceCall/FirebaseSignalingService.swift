...
protocol FirebaseSignalingDelegate: AnyObject {
    ...
    /// Remote peer changed camera state
    func signalingService(_ service: FirebaseSignalingService, remoteCameraStateChanged cameraOff: Bool, fromPeer peerId: String)
}
...
final class FirebaseSignalingService {
    ...
    private func listenForSignaling() {
        ...
        signalingHandle = ref.child("rooms").child(roomId).child("signaling")
            .observe(.childAdded) { [weak self] snapshot in
                ...
                case .cameraState:
                    guard sender != self.myPeerId else { return }
                    let cameraOff = json["cameraOff"] as? Bool ?? false
                    self.delegate?.signalingService(self, remoteCameraStateChanged: cameraOff, fromPeer: sender)
                }
            }
    }
    ...
    func sendCameraState(cameraOff: Bool) {
        let message: [String: Any] = [
            "type": SignalingMessageType.cameraState.rawValue,
            "sender": myPeerId,
            "receiver": "all",
            "cameraOff": cameraOff
        ]
        sendMessage(message)
        NSLog("📤 [Signaling] Sent cameraState (off=\(cameraOff)) to all")
    }
