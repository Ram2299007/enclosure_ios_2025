...
struct CallLogUserInfo: Identifiable, Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let stringValue = try? container.decodeIfPresent(String.self, forKey: .friendId) {
            friendId = stringValue
        } else if let intValue = try? container.decodeIfPresent(Int.self, forKey: .friendId) {
            friendId = String(intValue)
        } else {
            friendId = ""
        }
