...
final class NativeVideoCallSession: ObservableObject {
    ...
    @Published var isRemoteCameraOff = false
    ...
    func toggleCamera() {
        ...
        signalingService?.sendCameraState(cameraOff: isCameraOff)
...
extension NativeVideoCallSession: FirebaseSignalingDelegate {
    ...
    func signalingService(_ service: FirebaseSignalingService, remoteCameraStateChanged cameraOff: Bool, fromPeer peerId: String) {
        NSLog("📹 [VideoSession] Remote camera state changed: \(cameraOff ? "OFF" : "ON") from \(peerId)")
        DispatchQueue.main.async { [weak self] in
            self?.isRemoteCameraOff = cameraOff
        }
    }
}
#endif
