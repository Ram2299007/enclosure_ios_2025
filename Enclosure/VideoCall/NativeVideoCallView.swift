...
struct NativeVideoCallView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                // ── Caller name overlay (hidden when remote camera off overlay is shown) ──
                callerNameOverlay
                    .allowsHitTesting(false)
                    .opacity((showControls && !(session.isCallConnected && session.isRemoteCameraOff)) ? 1 : 0)
                    .animation(.easeInOut(duration: 0.4), value: showControls)
                    .animation(.easeInOut(duration: 0.3), value: session.isRemoteCameraOff)
    ...
    // Voice call design overlay - matches NativeVoiceCallScreen when camera is off
    private var voiceCallDesignOverlay: some View {
        ZStack {
            // Background - matches voice call background
            Color.black.ignoresSafeArea()
            ...
            // Caller info - matches voice call layout
            VStack(spacing: 0) {
                Spacer().frame(height: 180) // matches Android: layout_marginTop="180dp"
                
                // Caller photo
                AsyncImage(url: URL(string: session.payload.receiverPhoto)) { phase in
                    switch phase {
    ...
    private var fullScreenVideo: some View {
        Group {
            if session.isCallConnected && session.isRemoteCameraOff {
                // Connected + remote camera off: show remote peer's profile
                voiceCallDesignOverlay
            } else if session.isCallConnected, let remote = session.remoteRenderer {
                // Always show remote video when connected (regardless of local camera state)
                EAGLVideoViewWrapper(view: remote)
                    .ignoresSafeArea()
            } else if session.isCameraOff {
                // Not connected + camera off: show voice call design instead of black
                voiceCallDesignOverlay
    ...
    private func firstLetterAvatar(name: String) -> some View {
        ...
        let colors: [Color] = [
            ...
            Color(red: 0.96, green: 0.49, blue: 0.0),
            Color(red: 0.0, green: 0.59, blue: 0.53),
            Color(red: 0.91, green: 0.26, blue: 0.21),
            Color(red: 0.48, green: 0.32, blue: 0.66),
            Color(red: 0.0, green: 0.47, blue: 0.42),
        ]
