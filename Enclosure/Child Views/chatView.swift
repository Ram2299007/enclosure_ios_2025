...
struct chatView: View {
    ...
    // Navigation state for DummyChattingScreen
    @State private var navigateToDummyChatScreen: Bool = false
    ...
    var body: some View {
        ...
        return VStack(spacing: 0) {
            if viewModel.isLoading && !viewModel.hasCachedChats {
                ...
                List {
                    PlaceholderContactCardView(navigateToDummyChatScreen: $navigateToDummyChatScreen)
        ...
        .background(
            VStack(spacing: 0) {
                ...
                // Hidden NavigationLink for DummyChattingScreen
                NavigationLink(
                    destination: DummyChattingScreen(),
                    isActive: $navigateToDummyChatScreen
                ) {
                    EmptyView()
                }
                .hidden()
    ...
    struct PlaceholderContactCardView: View {
        ...
        @Binding var navigateToDummyChatScreen: Bool
        ...
        var body: some View {
            ...
            .gesture(
                DragGesture(minimumDistance: 0)
                    ...
                    .onEnded { _ in
                        isPressed = false
                        // Open dummy chat screen when placeholder card is tapped
                        navigateToDummyChatScreen = true
                    }
            )
