...
struct LockScreen2View: View {
    ...
    @State private var shouldShowLockUI: Bool = false
    ...
    var body: some View {
                NavigationStack {
                                                    ...
                                                    Constant.showToast(message: msg)
                                                        ...
                                                        if msg == "Screen unlocked !" || msg.contains("unlocked") {
                                                            ...
                                                            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                                                                print("🔓 [LockScreen2View] Starting transition to MainActivityOld")
                                                                // Show toast message before navigation
                                                                Constant.showToast(message: "Opening main app...")
                    ...
                    .opacity(shouldShowLockUI ? 1.0 : 0.0)
                    ...
                    .background(
                        ...
                        .animation(.easeInOut(duration: 0.5), value: shouldNavigateToMain) // Add smooth animation
                        .onChange(of: shouldNavigateToMain) { newValue in
                            ...
                            if newValue {
                                print("🔍 [LockScreen2View] Starting smooth transition animation")
                            }
            }
                    )
        }
    ...
    private func checkSleepModeStatus() {
        ...
        // If sleepKeyCheckOFF != "on" AND user is logged in, navigate to MainActivityOld immediately (matching Android onStart())
        if sleepKeyCheckOFF != "on" && !uid.isEmpty && uid != "0" {
            print("🔍 [LockScreen2View] Condition met: sleepKeyCheckOFF != 'on' AND uid is valid")
            print("🔍 [LockScreen2View] Navigating to MainActivityOld immediately (matching Android)")
            // Navigate immediately like Android onStart() - no delay
            shouldNavigateToMain = true
        } else {
            ...
            // Show lock screen UI only when user needs to unlock
