...
struct MainActivityOld: View {
    ...
    // Theme change dialog state (matching Android theme_dialogue)
    @State private var showThemeChangeDialog = false
    @State private var fetchedThemeColorFromAPI: String? = nil
    @State private var hasCheckedThemeColor = false
    @State private var pendingThemeDialogReady = false
    ...
    /// True when any dialog/overlay is showing that should hide the logo and toolbar
    private var isAnyDialogShowing: Bool {
        showNameDialog || showChatLongPressDialog || showCallLogDialog || showVideoCallLogDialog || showGroupDialog || showClearLogDialog || showClearVideoCallLogDialog
    }

    /// Toolbar item opacity — smoothly fades in/out based on isMainContentVisible
    private var toolbarItemOpacity: Double {
    ...
    var body: some View {
        ...
        .onChange(of: showNameDialog) { isShowing in
            ...
            if !isShowing && pendingThemeDialogReady {
                pendingThemeDialogReady = false
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                    withAnimation(.easeInOut(duration: 0.2)) {
                        showThemeChangeDialog = true
                    }
                    print("🎨 [MainActivityOld] Name dialog dismissed - now showing theme change dialog")
                }
            }
        }
    ...
    // MARK: - Fetch Theme Color from Server (matching Android fetchThemeColor in Webservice.java)
    private func fetchThemeColorFromServer() {
        guard !hasCheckedThemeColor else {
            print("🎨 [MainActivityOld] fetchThemeColor already checked, skipping")
            return
        }
        hasCheckedThemeColor = true
        
        let uid = Constant.SenderIdMy
        guard uid != "0" else {
            print("🎨 [MainActivityOld] fetchThemeColor skipped - no UID")
            hasCheckedThemeColor = false
            return
        }
        
        ApiService.fetch_theme_color(uid: uid) { success, message, colorCode in
            DispatchQueue.main.async {
                if success, let colorCode = colorCode {
                    print("🎨 [MainActivityOld] Theme color fetched: \(colorCode)")
                    
                    // Only show dialog if theme color is NOT default AND differs from already-saved theme
                    let savedTheme = UserDefaults.standard.string(forKey: Constant.ThemeColorKey) ?? "#00A3E9"
                    if colorCode != "#00A3E9" && colorCode != savedTheme {
                        fetchedThemeColorFromAPI = colorCode
                        // Defer theme dialog if name dialog is currently showing
                        if showNameDialog {
                            pendingThemeDialogReady = true
                            print("🎨 [MainActivityOld] Name dialog is showing - deferring theme dialog")
                        } else {
                            withAnimation(.easeInOut(duration: 0.2)) {
                                showThemeChangeDialog = true
                            }
                            print("🎨 [MainActivityOld] Showing theme change dialog for color: \(colorCode)")
                        }
                    } else {
                        print("🎨 [MainActivityOld] Theme is default or already applied (\(colorCode)), no dialog needed")
                    }
                } else {
                    print("🎨 [MainActivityOld] fetchThemeColor failed: \(message)")
                }
            }
        }
    }
    
    // MARK: - Change App Icon for Theme (matching Android changeAppIcon in Webservice.java)
    private func changeAppIconForTheme(_ themeColor: String) {
        guard UIApplication.shared.supportsAlternateIcons else {
            print("🎨 [MainActivityOld] Alternate icons not supported")
            return
        }
        
        let iconName = getAlternateIconName(for: themeColor)
        print("🎨 [MainActivityOld] Changing app icon for theme: \(themeColor) -> icon: \(iconName ?? "default")")
        
        // Swizzle present(_:animated:completion:) to suppress the system alert (matching ThemeView.swift)
        ...
        UIApplication.shared.setAlternateIconName(iconName) { error in
            ...
            if let error = error {
                print("🎨 [MainActivityOld] Error changing app icon: \(error.localizedDescription)")
            } else {
                print("🎨 [MainActivityOld] App icon changed successfully")
            }
        }
    }
    
    // Get alternate icon name for theme color (matching Android alias names)
