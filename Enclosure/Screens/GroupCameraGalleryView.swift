        ...
        if currentStatus == .notDetermined {
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { status in
                if status == .authorized || status == .limited {
                    ...
                    print("GroupCameraGalleryView: Photo library permission denied")
            ...
            print("GroupCameraGalleryView: Photo library permission not available")
        ...
        if currentStatus == .notDetermined {
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { status in
                if status == .authorized || status == .limited {
                    ...
                    print("GroupCameraGalleryView: Photo library permission denied for video")
                }
            }
        } else if currentStatus == .authorized || currentStatus == .limited {
            performSaveVideo(url: url)
        } else {
            print("GroupCameraGalleryView: Photo library permission not available for video")
        }
    }
