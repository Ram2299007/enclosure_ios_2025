    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                .animation(.spring(response: 0.35, dampingFraction: 0.7), value: showScrollDownButton)
...
struct MessageBubbleView: View {
    ...
    var body: some View {
        ...
        .onLongPressGesture(minimumDuration: 0.5, maximumDistance: 20) {
            ...
            // Use the bubble's global frame midY as the touch Y — accurate enough for positioning
...
struct MessageLongPressDialog: View {
    ...
    @State private var measuredDialogHeight: CGFloat = 360 // Actual rendered dialog height for boundary clamping
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .topLeading) {
                    ...
                    GeometryReader { geo -> Color in
                        DispatchQueue.main.async {
                            if geo.size.height > 0 {
                                measuredDialogHeight = geo.size.height
                            }
                        }
                        return Color.clear
                    }
                )
                .allowsHitTesting(true) // Allow touches on dialog content
                ...
                .position(
                    x: isSentByMe
                        ? geometry.size.width - 10 - 155  // 10px from right edge, 155 = half of 310
                        : 10 + 155,                        // 10px from left edge
                    y: adjustedCenterY(in: geometry)
                )
    ...
    // Calculate center Y for .position() — places dialog at exact touch location, clamped to comfort zone
    private func adjustedCenterY(in geometry: GeometryProxy) -> CGFloat {
        let dialogHeight = measuredDialogHeight
        let topPadding: CGFloat = 12
        let bottomPadding: CGFloat = 24

        // If position is invalid, center dialog on screen
        guard position.y > 0 else {
            return geometry.size.height / 2
        }

        let frame = geometry.frame(in: .global)
        // Convert global touch Y → local Y within GeometryReader
        let localY = position.y - frame.minY

        // Ideal: place dialog so emoji strip (~30pt from top) aligns with touch point
        let emojiStripOffset: CGFloat = 30
        let desiredTop = localY - emojiStripOffset

        // Clamp so dialog stays within screen boundaries
        let minTop = topPadding
        let maxTop = geometry.size.height - dialogHeight - bottomPadding
        let clampedTop = min(max(desiredTop, minTop), max(minTop, maxTop))

        // .position() uses center point, so convert top edge to center
        return clampedTop + dialogHeight / 2
    }
