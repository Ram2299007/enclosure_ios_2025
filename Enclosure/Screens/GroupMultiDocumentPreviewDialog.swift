//
//  GroupMultiDocumentPreviewDialog.swift
//  Enclosure
//
...
struct GroupMultiDocumentPreviewDialog: View {
    ...
    var body: some View {
                    ...
                    HStack(spacing: 0) {
                        ...
                        VStack(spacing: 0) {
                            Button(action: {
                                ...
                                print("GroupMultiDocumentPreviewDialog: Send button clicked - Caption: '\(trimmedCaption)' (length: \(trimmedCaption.count))")
        ...
        .onAppear {
            print("GroupMultiDocumentPreviewDialog: onAppear - documents count: \(selectedDocuments.count)")
            print("GroupMultiDocumentPreviewDialog: documents: \(selectedDocuments.map { $0.lastPathComponent })")
            print("GroupMultiDocumentPreviewDialog: onAppear - Initial caption: '\(caption)' (length: \(caption.count))")
    ...
    private func handleMultiDocumentSend(caption: String) {
        print("GroupMultiDocumentPreviewDialog: === MULTI-DOCUMENT SEND ===")
        print("GroupMultiDocumentPreviewDialog: Selected documents count: \(selectedDocuments.count)")
        print("GroupMultiDocumentPreviewDialog: Caption: '\(caption)'")
        
        guard !selectedDocuments.isEmpty else {
            print("GroupMultiDocumentPreviewDialog: No documents selected, returning")
            return
        }
        ...
        let lockQueue = DispatchQueue(label: "com.enclosure.groupMultiDocumentUpload.lock")
        ...
        dispatchGroup.notify(queue: .main) {
            if uploadResults.isEmpty {
                print("❌ [GROUP_MULTI_DOCUMENT] Upload failed - no results")
                Constant.showToast(message: "Unable to upload documents. Please try again.")
                return
            }
            
            if !uploadErrors.isEmpty {
                print("⚠️ [GROUP_MULTI_DOCUMENT] Some uploads failed: \(uploadErrors.count) errors")
            }
            ...
            for (index, result) in sortedResults.enumerated() {
                ...
                print("GroupMultiDocumentPreviewDialog: Creating ChatMessage \(index + 1)/\(sortedResults.count) with caption: '\(documentCaption)'")
                ...
                print("GroupMultiDocumentPreviewDialog: ChatMessage created with caption: '\(newMessage.caption ?? "nil")'")
                ...
                MessageUploadService.shared.uploadMessage(
                    ...
                    if success {
                        print("✅ [GROUP_MULTI_DOCUMENT] Uploaded document \(index + 1)/\(sortedResults.count) for modelId=\(documentModelId)")
                        
                        // Check if message exists in Firebase and stop progress bar (matching Android)
                        self.checkMessageInFirebaseAndStopProgress(messageId: documentModelId, groupId: groupId)
                        ...
                        print("❌ [GROUP_MULTI_DOCUMENT] Upload error: \(errorMessage ?? "Unknown error")")
