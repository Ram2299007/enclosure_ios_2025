...
struct whatsTheCode: View {
    ...
    init(uid: String, c_id: String, mobile_no: String, country_Code: String) {
        ...
        // Validate parameters - if empty, try to get from UserDefaults
        if uid.isEmpty || mobile_no.isEmpty {
            // print("⚠️ [whatsTheCode] Empty parameters detected, using UserDefaults fallback")
            self.uid = UserDefaults.standard.string(forKey: Constant.UID_KEY) ?? uid
            self.c_id = UserDefaults.standard.string(forKey: Constant.c_id) ?? c_id
            self.mobile_no = UserDefaults.standard.string(forKey: Constant.PHONE_NUMBERKEY) ?? mobile_no
            self.country_Code = UserDefaults.standard.string(forKey: Constant.country_Code) ?? country_Code
            // print("🔄 [whatsTheCode] Fallback - UID: \(self.uid), Mobile: \(self.mobile_no)")
        }
